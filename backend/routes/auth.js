...
// Logs SendGrid-specific error details so failures are always diagnosable.
function logEmailError(context, err) {
      console.error(`[email] ${context} failed — code: ${err.code ?? 'n/a'}, errors:`,
...
router.post('/register', validateRegister, async (req, res) => {
      ...
      try {
        ...
        // Account is created — email failure must NOT roll back registration.
        try {
                  await sendVerificationEmail(email, verificationCode);
        } catch (emailErr) {
                  logEmailError('register/sendVerificationEmail', emailErr);
                  // Account exists; client can request a resend. Surface a clear non-500 message.
                return res.status(201).json({
                            message: 'Account created but confirmation email could not be sent. Use Resend Verification to try again.',
...
router.post('/resend-verification', async (req, res) => {
      ...
      if (!email) return res.status(400).json({ error: 'Email is required' });
        ...
        try {
                  ...
                  logEmailError('resend-verification/sendVerificationEmail', emailErr);
                  return res.status(502).json({ error: 'Verification email could not be sent. Please try again shortly.' });
        }
      ...
      try {
        ...
        if (!user.email_verified) {
                  ...
                  try {
                              ...
                              logEmailError('login/sendVerificationEmail', emailErr);
                              // Don't 500 — user is unverified but credentials are valid. Tell them to use resend.
                    return res.status(403).json({
                                  error: 'Email not verified. Verification email could not be sent — use Resend Verification.',
                                  requiresVerification: true,
                                  email: user.email,
                    });
                  }
