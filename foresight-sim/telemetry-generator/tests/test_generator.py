...
def test_sample_rates_respected():
    ...
    """
    profile = make_vehicle_profile("RATE-001", "sedan_low_mileage", seed=7)
...
def test_most_values_within_healthy_range():
    ...
    Pre-warm 600 ticks to advance past cold-start phase (tick_of_day < 600),
    which intentionally produces below-healthy coolant/catalyst values.
    """
...
def test_override_replaces_value(gen):
    ...
    override_val = 750.0  # Within rpm's critical_range (300–7500); clearly distinct from normal
    for _ in range(10):
...
def test_run_day_returns_correct_count():
    ...
    profile = make_vehicle_profile("DAY-001", "sedan_low_mileage", seed=5)
    gen = TelemetryGenerator(profile)
...
def test_run_day_advances_day_counter():
    profile = make_vehicle_profile("DAY-002", "sedan_low_mileage", seed=6)
    gen = TelemetryGenerator(profile)
    assert gen.day == 0
    gen.run_day()
    assert gen.day == 1
...
def test_run_days_callback_called_per_day():
    profile = make_vehicle_profile("CB-001", "sedan_low_mileage", seed=8)
    gen = TelemetryGenerator(profile)
    days_seen = []
...
def test_cold_start_profile_at_tick_0():
    ...
    from datetime import datetime
    profile = make_vehicle_profile("COLD-001", "sedan_low_mileage", seed=11)
