...
class FailureInjector:
    ...
    def tick(self) -> Tuple[List[TelemetryReading], FailureLabel]:
        ...
        if not self._dtc_fired and factor > 0.0:
            # Estimate remaining days by solving curve(x) = dtc_threshold
            # Simple: scan forward to find when threshold is crossed
            days_to_dtc = self._estimate_days_to_dtc(day)
