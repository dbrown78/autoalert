...
// ─── Scan card ───────────────────────────────────────────────────────────────
function ScanCard({ scan, onDelete, onPress, loading = false }) {
...
export default function ScanHistoryScreen({ navigation }) {
  ...
  const [loadingId, setLoadingId] = useState(null); // scan being tapped
  ...
  const handleScanPress = useCallback(async (scan) => {
    if (loadingId) return;
    setLoadingId(scan.id);
    try {
      const res = await client.get(`/scans/${scan.id}`);
      navigation.navigate('DTCDetail', { code: scan.dtc_code, scan: res.data.scan });
    } catch {
      Alert.alert('Error', 'Could not load scan details.');
    } finally {
      setLoadingId(null);
    }
  }, [loadingId, navigation]);
