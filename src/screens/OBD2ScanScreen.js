...
function StatusBar({ status, adapterVersion }) {
  const dot =
  ...
  const label =
    status === 'connected'
      ? `CONNECTED${adapterVersion ? ` — ${adapterVersion.slice(0, 14)}` : ''}`
...
export default function OBD2ScanScreen({ navigation }) {
  ...
  const [bleDtcCodes, setBleDtcCodes]   = useState([]);
  ...
  const sensorBatchRef   = useRef([]);

  const {
    bleState, lastDTC, isSupported,
  ...
  // ── OBD2 Protocol: init ELM327 + poll PIDs after BLE connect ─────────────
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const { success, adapterVersion: ver, errors } = await initELM327(sendCommand);
      if (cancelled) return;

      if (!success) {
        setInitError(errors[0] ?? 'ELM327 init failed');
        return;
      }
      setAdapterVersion(ver);
      setInitError(null);

      // Read stored (Mode 03) and pending (Mode 07) DTCs on connect
      const [stored, pending] = await Promise.all([
        requestDTCs(sendCommand),
        requestPendingDTCs(sendCommand),
      ]);
      if (!cancelled) {
        const combined = [...new Set([...stored, ...pending])];
        setBleDtcCodes(combined);
      }
      ...
      pollIntervalRef.current = setInterval(async () => {
        if (cancelled) return;
        try {
          const results = await pollAllPIDs(sendCommand);
      ...
      // Flush sensor buffer to backend every 5s
      batchIntervalRef.current = setInterval(async () => {
        if (!vehicleId || !sensorBatchRef.current.length) return;
        const batch = sensorBatchRef.current.splice(0);
        try {
          await client.post(`/sensors/${vehicleId}/batch`, { readings: batch });
        } catch {
          // Retain failed readings for next flush
          sensorBatchRef.current.unshift(...batch);
        }
      }, 5000);
    };
  ...
  }, [bleState, sendCommand, vehicleId]);

  // ── Accumulate DTC events from the native layer ───────────────────────────

  useEffect(() => {
    if (!lastDTC?.code) return;
    setDtcs(prev => {
      if (prev.find(d => d.code === lastDTC.code)) return prev;
      return [...prev, lastDTC];
    });
    setScanned(true);
    // Persist to backend
    client.post('/scans', {
      dtc_code:   lastDTC.code,
  ...
  }, [isSupported, bleState, disconnect]);
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      if (isSupported) {
        requestManualScan();
        // Results arrive via onDTCDetected events — set a timeout to mark complete
        setTimeout(() => { setScanning(false); setScanned(true); }, 4000);
      } else {
        // Fallback: server-side scan record only
        const payload = selectedVehicle?.id ? { vehicle_id: selectedVehicle.id } : {};
        await client.post('/scans', payload);
        setScanned(true);
        setScanning(false);
      }
    } catch (e) {
  ...
  }, [isConnected, scanning, isSupported, requestManualScan, selectedVehicle?.id]);
  ...
  return (
    <View style={S.container}>
      ...
      <StatusBar status={adapterStatus} adapterVersion={adapterVersion} />
      ...
      <ScrollView style={S.body} contentContainerStyle={S.bodyContent}>
        ...
        {initError && (
          <View style={S.errorRow}>
            ...
            <Text style={S.errorText}>ADAPTER INIT: {initError}</Text>
          </View>
        )}
        ...
        {/* Live BLE DTCs from Mode 03/07 */}
        {isConnected && bleDtcCodes.length > 0 && (
          <>
            ...
            {bleDtcCodes.map((code, i) => (
              ...
              >
                ...
                <View style={S.dtcBody}>
                  ...
                  <Text style={S.dtcDesc}>Tap to look up details</Text>
                </View>
                <Text style={S.dtcArrow}>›</Text>
              </TouchableOpacity>
            ))}
      ...
      <View style={S.footerRow}>
        ...
        {isSupported && (
          <View style={[S.sourceBadge, { borderColor: isConnected ? C.green : '#303030' }]}>
            <Text style={[S.sourceText, { color: isConnected ? C.green : '#404040' }]}>
              {isConnected ? '● BLE LIVE' : '○ BLE READY'}
            </Text>
          </View>
        )}
      </View>
...
const S = StyleSheet.create({
  ...
  warnBanner: {
    ...
    borderWidth: 1, borderColor: 'rgba(192,139,48,0.25)',
  },
  warnStripe: { width: 3, backgroundColor: C.amber },
  warnText: {
    flex: 1, color: C.amber, fontSize: 10,
    padding: 12, letterSpacing: 0.5, lineHeight: 16,
  },
});
