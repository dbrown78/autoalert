...
export default function RegisterScreen({ navigation }) {
  ...
  return (
    <View style={S.container}>
      ...
      <TextInput style={S.input} placeholder="Email" placeholderTextColor="rgba(255,255,255,0.3)"
        value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address"/>
      <TextInput style={S.input} placeholder="Password" placeholderTextColor="rgba(255,255,255,0.3)"
        value={password} onChangeText={setPassword} secureTextEntry/>
      <TouchableOpacity style={[S.btn, loading && {opacity:0.6}]} onPress={() => register(name, email, password)} disabled={loading}>
